...
await request(
  ...
  const { data: { results } } = await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${latitude},${longitude}&radius=1000&type=cafe&key=${API_KEY}`)
    this.setState({
      places: results,
      fetching:false,
      refreshing:false
    });
    await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1002&type=cafe&key=${API_KEY}`)
...
handleRefresh = () => {
  this.setState(
    ...
    () => {
       axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=cafe&key=${API_KEY}`)
      .then(response=>this.setState({
...
axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=cafe&key=${API_KEY}&pagetoken=${this.state.pageToken}`)
.then(response=>this.setState({
 places: response.data.results,
 fetching:false,
 refreshing:false
