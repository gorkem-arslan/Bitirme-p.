...
await request(
    Platform.select({
      ...
      ios: PERMISSIONS.IOS.LOCATION_WHEN_IN_USE,
		...
		const { data: { results } } = await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${latitude},${longitude}&radius=1000&type=hotel&key=${API_KEY}`)
      this.setState({
				places: results,
        fetching:false,
        refreshing:false
			});
      await axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1002&type=hotel&key=${API_KEY}`)
  ...
  handleRefresh = () => {
    this.setState(
      ...
      () => {
         axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=hotel&key=${API_KEY}`)
        .then(response=>this.setState({
...
if (this.state.pageToken === undefined) {
  axios.get(`${API_ENDPOINT}/nearbysearch/json?location=${this.state.region.latitude},${this.state.region.longitude}&radius=1003&type=hotel&key=${API_KEY}&pagetoken=${this.state.pageToken}`)
 .then(response=>this.setState({
   places: response.data.results,
   fetching:false,
   refreshing:false
  ...
  focus(){
      ...
      setTimeout(() => {
          ...
          const GOOGLE_MAPS_APIKEY = 'AIzaSyDcyKd0OrhGccoKID0QZp2gBcOTXu_uhBY';
        ...
        {
          this.state.fetching ? <Text style={styles.loading}>Yakındaki Mekanlar Alınıyor...</Text> : <Places map={this.map} places={this.state.places} renderFooter={this.renderFooter}
